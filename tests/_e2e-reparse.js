...
async function main() {
  ...
  const page = targets.find(
    (t) =>
      t.type === 'page' &&
      t.url &&
      t.url.indexOf('doubao.com/chat/38427006775604738') >= 0
  )
  if (!page) {
        ...
        req.onsuccess = function() {
          ...
          tx.oncomplete = function() {
            var doubaoConv = convs.find(function(c) {
              return c.id && c.id.indexOf('doubao:38427006775604738') >= 0;
            });
      ...
      return new Promise(function(resolve) {
        chrome.tabs.query({}, function(tabs) {
          var d = tabs.find(function(t) {
            return t.url && t.url.indexOf('doubao.com/chat/38427006775604738') >= 0;
          });
