...
// --- 豆包: 真实 /im/chain/single 响应 ---
{
  ...
  if (fs.existsSync(fixturePath)) {
    ...
    const ev: IngestEvent = {
      source: "fetch-hook",
      site: "doubao",
      url: "https://www.doubao.com/im/chain/single?version_code=20800&device_id=1234567890123456789",
      body,
      capturedAt: Date.now()
    }
    const r = parseEvent(ev)
    ...
    assert("解析成功", !!r)
    assert("convId=33333333333333333", r?.conversation.conversationId === "33333333333333333")
    ...
    return JSON.stringify({
      downlink_body: {
        pull_singe_chain_downlink_body: {
          messages: msgs.map((m) => ({
            conversation_id: "11111111111111111",
            message_id: m.id,
            user_type: m.ut,
